...
export function LessonCard({ lesson }: { lesson: any }) {
  return (
    <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-transparent hover:border-gold/20 flex flex-col h-full transform hover:-translate-y-2">
      <div className="aspect-video relative overflow-hidden bg-black shrink-0">
        <img 
          ...
          onError={(e) => {
            (e.target as HTMLImageElement).src = `https://img.youtube.com/vi/${lesson.youtubeId}/mqdefault.jpg`
          }}
          alt={lesson.title} 
        ...
        <Badge className="absolute top-4 left-4 bg-maroon text-white border-maroon-light/50 shadow-lg px-3 py-1 text-[10px] uppercase font-bold tracking-widest backdrop-blur-sm font-sans">
          {lesson.category}
        </Badge>
      </div>
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <p className="text-gray-500 text-sm line-clamp-3 mb-6 font-inter font-light">
          {lesson.description || "Tune in to this enlightening Dhamma lesson and find peace in the Buddha's teachings."}
        </p>
        ...
        <div className="mt-auto pt-6 flex items-center justify-between border-t border-gray-50">
          ...
          <div className="flex gap-1">
            <button className="w-9 h-9 flex items-center justify-center rounded-full bg-gray-50 text-gray-400 hover:bg-yellow-50 hover:text-yellow-700 transition-all border border-transparent hover:border-yellow-200">
